    ...
    >
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-white ml-auto pl-3">{raw.cost_per_landing_page_view > 0 ? fCurrency(raw.cost_per_landing_page_view) : "—"}</span>
        </div>
  ...
  return (
    <div className="rounded-xl border border-border bg-card px-3 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mb-4">
        <h3 className="text-sm font-semibold text-foreground flex-1 min-w-0">Visitas a la Tienda y Costo por Visita</h3>
      ...
      {isMobile ? (
        ...
        <ResponsiveContainer width="100%" height={270}>
          <ComposedChart data={data} margin={{ top: 10, right: 20, bottom: 0, left: 10 }}>
            ...
            <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 11, fill: TICK_COLOR }} axisLine={false} tickLine={false} allowDecimals={false} domain={[0, Math.ceil(maxVisits * 1.4)]} dx={-4} width={30} />
            <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: TICK_COLOR }} axisLine={false} tickLine={false} domain={[0, Math.ceil(maxCpv * 1.4)]} tickFormatter={(v: number) => v >= 1000 ? `$${new Intl.NumberFormat("es-AR").format(Math.round(v / 1000))}k` : `$${v}`} dx={4} width={48} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(255,255,255,0.03)" }} />
            <Bar yAxisId="left" dataKey="landing_page_views" fill={BAR_COLOR} fillOpacity={0.8} radius={[4, 4, 0, 0]} maxBarSize={52} label={<BarLabel />} isAnimationActive={false} />
            <Line isAnimationActive={false} yAxisId="right" dataKey="cost_per_landing_page_view" stroke={LINE_COLOR} strokeWidth={2} strokeDasharray="5 3" dot={{ fill: LINE_COLOR, r: 3.5, strokeWidth: 0 }} activeDot={{ r: 5.5, fill: LINE_COLOR, strokeWidth: 0 }} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      )}
    </div>
