...
import { getGoogleSummary } from "@/lib/google-ads/client";
...
function comparisonDates(since: string, until: string): { since: string; until: string } {
...
export async function GET(
  ...
  try {
    ...
    const { data: client } = await admin
      .from("clients")
      .select("id, google_ads_account_id, google_ads_access_token")
      .eq("id", accountId)
      .single();
    ...
    const cmp = comparisonDates(dates.since, dates.until);

    const [current, comparison] = await Promise.all([
      getGoogleSummary(client.google_ads_access_token, dates.since, dates.until),
      getGoogleSummary(client.google_ads_access_token, cmp.since, cmp.until),
    ]);

    return NextResponse.json({ current, comparison });
  } catch (err) {
    console.error("[API /google/[accountId]]", err);
    return NextResponse.json({ error: "Error al obtener datos de Google Ads" }, { status: 500 });
  }
}
